...
Here's a minimal [_docker-compose.yaml_](https://github.com/LimKianAn/x-cellent-cloudgateway/tree/main/demo/minimal/docker-compose.yaml).
...
Another pillar of Cloud-Gateway is _pipe_, which can be found in Cloud-Gateway's configuration of both the [server](https://github.com/LimKianAn/x-cellent-cloudgateway/tree/main/demo/minimal/server/conf.yaml) and the [client](https://github.com/LimKianAn/x-cellent-cloudgateway/tree/main/demo/minimal/client/conf.yaml). Remember that this file is consumed by Cloud-Gateway via _volumes_ in the _docker-compose.yaml_ above.
...
How about if we want to reach a service in the network _cloudnative_ from a client in the network _legacy_? _Reverse pipe_ comes to rescue. In contrast to normal pipes, where a service is resolvable within the server's network without going through the VPN to another network, a revere pipe is for the scenario where a service does sit in another network and the server needs the VPN to fetch it. Here's the [_docker-compose.yaml_](https://github.com/LimKianAn/x-cellent-cloudgateway/tree/main/demo/reverse/docker-compose.yaml) for such a reverse pipe.
...
Finally, let's check the _cong.yaml_ of Cloud-Gateway for each container. Note that for the [server](https://github.com/LimKianAn/x-cellent-cloudgateway/tree/main/demo/reverse/server/conf.yaml) and the [client](https://github.com/LimKianAn/x-cellent-cloudgateway/tree/main/demo/reverse/client/cloudnative/conf.yaml) in the network _cloudnative_, we have to specify the peer:
...
However, for the [client](https://github.com/LimKianAn/x-cellent-cloudgateway/tree/main/demo/reverse/client/legacy/conf.yaml) in the network _legacy_, it's the same as the previous demo:
